...
function guardarCliente(){
    ...
    const camposVacios = [mesa, hora].some(campo => campo == ''); //valido si los campos estan vacios con el some
    ...
    if(camposVacios){
        ...
        //en caso de tener todos los campos llenos
...
function agregarOrden(producto){
    ...
    if(producto.cantidad > 0){
        //estoy validando que el producto esté o exista
        if(pedido.some(item => item.id === producto.id)){
            ...
            //en caso de que no exista el item lo agregamos como nuevo
            cliente.pedido = [...pedido, producto];
        }
    }else{
        //cantidad sea igual a cero
...
function actualizarResumen(){
    ...
    //mostrarlos items del menu consumidos por el cliente
